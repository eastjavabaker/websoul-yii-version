  <h3> Modules &raquo; Article &raquo; Edit</h3>
<form action="<?php echo site_url("websoul/article/edit"); ?>" id="editform" name="editform" method="post" enctype="multipart/form-data">            
	    <fieldset>
            <table>              
              <?php foreach($query as $v){
		    if($v->publish_date_from && $v->publish_date_to){
		       $arrfrom = explode("-", $v->publish_date_from);
		       $arrto = explode("-", $v->publish_date_to);
		    }
		?>
                <tr>
                ...
                <tr valign="top">
                  ...
                  <td><input type="text" name="title" id="title" class="text-input large-input" value="<?php echo $v->title;?>" />
		  </td>
                </tr>
		<tr>
                  <td width="120" valign="top"><strong>Short Content *</strong></td>
                  <td valign="top"><textarea rows="3" cols="100" name="txtsortcontent" id="txtsortcontent" class="textarea" /><?php echo $v->short_content;?></textarea></td>
                </tr>
                <tr>
                  <td width="120"  valign="top" ><strong>Content *</strong></td>
                  <td  valign="top"><textarea rows="5" cols="100" name="txtcontent" id="txtcontent" class="textarea" /><?php echo $v->content;?></textarea><div id="contentnotif" class="errorx" style="font-weight: bold;" >Please enter an article content.</div></td>
                </tr>
                <?php if($v->picture!=""){  ?>
                <tr>
                  <td valign="top" bgcolor="#cccccc">&nbsp;</td>
                  <td bgcolor="#cccccc"><img src="<?php echo site_url('data/articles')."/".$v->picture;?>" /></td>
                </tr>
                <?php } ?>
                <tr>
                ...
                <tr valign="top">
                  ...
                  <td><input type="text" name="seotitle" id="seotitle" class="text-input large-input" value="<?php echo $v->title;?>" />
		  </td>
                </tr>
		<tr>
                  <td width="120" valign="top"><strong>Meta Description</strong></td>
                  <td valign="top"><textarea rows="5" cols="100" name="meta_desc" id="meta_desc" class="textarea" /><?php echo $v->meta_description;?></textarea></td>
                </tr>
		<tr>
                  <td width="120" valign="top"><strong>Meta Keywords</strong></td>
                  <td valign="top"><textarea rows="5" cols="100" name="meta_key" id="meta_key" class="textarea" /><?php echo $v->meta_keyword;?></textarea></td>
                </tr>	
		<tr valign="top">
...
<select id="dayfrom" name="dayfrom">
    ...
    <?php for($d=1;$d<=31;$d++){?>
    <option value="<?php echo $d;?>" <?php echo ($d==$arrfrom[2])?'selected="selected"':"";?> ><?php echo $d;?></option>
    <?php }?>
</select>

<select id="monthfrom" name="monthfrom">
    <?php foreach($arrmonth as $key=>$month){ ?>
    <option value="<?php echo $key;?>" <?php echo ($key==$arrfrom[1])?'selected="selected"':"";?>><?php echo $month;?></option>
    <?php }?> 
</select>
...
<select id="yearfrom" name="yearfrom">
    ...
    <option value="<?php echo $e;?>"  <?php echo ($e==$arrfrom[0])?'selected="selected"':"";?>><?php echo $e;?></option>
    <?php }?>
</select> - <strong>To :</strong>
<select id="dayto" name="dayto">
    <option value="0">-Day-</option>
    <?php for($d=1;$d<=31;$d++){?>
    <option value="<?php echo $d;?>" <?php echo ($d==$arrto[2])?'selected="selected"':"";?>><?php echo $d;?></option>
    <?php }?>
</select>

<select id="monthto" name="monthto">
    <?php foreach($arrmonth as $key=>$month){ ?>
    <option value="<?php echo $key;?>" <?php echo ($key==$arrto[1])?'selected="selected"':"";?>><?php echo $month;?></option>
    <?php }?>
</select>
...
<select id="yearto" name="yearto">
    <option value="0">-Year-</option>
    <?php for($e=2012;$e<=2015;$e++){?>
    <option value="<?php echo $e;?>" <?php echo ($e==$arrto[0])?'selected="selected"':"";?>><?php echo $e;?></option>
    <?php }?>
</select>
		</td>
                </tr>
